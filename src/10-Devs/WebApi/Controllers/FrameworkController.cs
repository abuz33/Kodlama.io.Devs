...
public class FrameworkController : BaseController
{
    ...
    {
        var result = await Mediator.Send(new GetFrameworkListQuery() { PageRequest = pageRequest });
        return Ok(result);
    }

    [HttpGet("{Id}")]
    public async Task<IActionResult> Get ([FromRoute] GetFrameworkByIdQuery getByIdFrameworkQuery)
    {
